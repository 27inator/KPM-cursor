...
import { generateWallet, fundWallet } from './services/kaspa-wasm-wallet';
...
const WALLET_DB = './company_wallets.json';

function saveWallet(id: string, mnemonic: string, address: string, privKey: string) {
  let db: Record<string, any> = {};
  if (fs.existsSync(WALLET_DB)) db = JSON.parse(fs.readFileSync(WALLET_DB, 'utf8'));
  db[id] = {
    mnemonic: encrypt(mnemonic),
    address,
    privKey: encrypt(privKey)
  };
  fs.writeFileSync(WALLET_DB, JSON.stringify(db, null, 2));
}
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // User purchases
  app.post("/api/users/me/purchases", authenticateToken, async (req, res) => {
    try {
  ...
  app.get("/api/users/me/purchases", authenticateToken, async (req, res) => {
    try {
      const userId = (req as any).user?.id;
  ...
  app.get("/api/v1/users/me/purchases/:id/certificate", authenticateToken, async (req, res) => {
    try {
      const { id } = req.params;
      const userId = (req as any).user?.id;
  ...
  app.put("/api/companies/:companyId/policy", authenticateToken, async (req, res) => {
    try {
      const { companyId } = req.params;
      const { visibleFields, commitEventTypes, reason } = req.body;
      const adminUserId = (req as any).user?.id?.toString() || "unknown";
  ...
  app.get("/api/policy/audits", authenticateToken, async (req, res) => {
    try {
  ...
  app.get("/api/policy/audits/:companyId", authenticateToken, async (req, res) => {
    try {
      const { companyId } = req.params;
      const limit = parseInt(req.query.limit as string) || 50;
  ...
  app.put("/api/alerts/:alertId/acknowledge", authenticateToken, async (req, res) => {
    try {
  ...
  app.get("/api/analytics/company/:companyId", authenticateToken, async (req, res) => {
    try {
      const { companyId } = req.params;
      const { from, to } = req.query;
      const startDate = from ? new Date(from as string) : new Date(Date.now() - 30 * 24 * 60 * 60 * 1000);
  ...
  app.get("/api/analytics/system/export", authenticateToken, async (req, res) => {
    try {
  ...
  app.get("/api/analytics/company/:companyId/export", authenticateToken, async (req, res) => {
    try {
      const { companyId } = req.params;
      const { format = 'csv', from, to } = req.query;
      const startDate = from ? new Date(from as string) : new Date(Date.now() - 30 * 24 * 60 * 60 * 1000);
  ...
  app.put("/api/notifications/mark-all-read", authenticateToken, async (req, res) => {
    try {
      ...
      await storage.markAllNotificationsAsRead((req as any).user?.id, companyId);
  ...
  app.post("/api/notifications/create-sample", authenticateToken, async (req, res) => {
    try {
      ...
      for (const notification of sampleNotifications) {
        const created = await storage.createNotification(notification as any);
  ...
  app.get("/api/error-logs/operation/:operationName", authenticateToken, async (req, res) => {
    try {
  ...
  app.put("/api/error-logs/:errorId/resolve", authenticateToken, async (req, res) => {
    try {
  ...
  app.get("/api/dead-letter-queue", authenticateToken, async (req, res) => {
    try {
  ...
  app.put("/api/dead-letter-queue/:operationId/status", authenticateToken, async (req, res) => {
    try {
  ...
  app.delete("/api/dead-letter-queue/:operationId", authenticateToken, async (req, res) => {
    try {
      const { operationId } = req.params;
  ...
  app.get("/api/system-metrics/time-range", authenticateToken, async (req, res) => {
    try {
  ...
  app.get("/api/error-handler/stats", authenticateToken, async (req, res) => {
    try {
  ...
  app.get("/api/error-handler/failed-operations", authenticateToken, async (req, res) => {
    try {
  ...
  app.post("/api/test/blockchain-error", authenticateToken, async (req, res) => {
    try {
  ...
  app.get('/api/system/wallet-test', async (req, res) => {
    try {
  ...
  app.get("/api/system/performance-report", authenticateToken, async (req, res) => {
    try {
      const { monitoringService } = await import("./services/monitoring");
  ...
  app.get("/api/system/metrics/recent", authenticateToken, async (req, res) => {
    try {
      const limit = parseInt(req.query.limit as string) || 50;
      const metricType = req.query.type as string;
  ...
  app.get("/api/security/compliance-metrics", authenticateToken, async (req, res) => {
    try {
  ...
  app.get("/api/security/compliance-report", authenticateToken, async (req, res) => {
    try {
      const { complianceService } = await import("./services/compliance");
  ...
  app.put("/api/security/incidents/:id/resolve", authenticateToken, async (req, res) => {
    try {
      const incidentId = parseInt(req.params.id);
      const resolvedBy = (req as any).user?.id?.toString();
  ...
  app.get("/api/security/export-compliance", authenticateToken, async (req, res) => {
    try {
      const { complianceService } = await import("./services/compliance");
      const startDate = new Date(req.query.startDate as string);
      const endDate = new Date(req.query.endDate as string);
  ...
  app.get("/api/system/metrics/recommendations", authenticateToken, async (req, res) => {
    try {
      const { monitoringService } = await import("./services/monitoring");
  ...
  app.get("/api/security/compliance-metrics", authenticateToken, async (req, res) => {
    try {
      ...
      const securityIncidents = incidents.filter(incident => 
        ...
        incident.createdAt && new Date(incident.createdAt) <= endDate
      ).length;
  ...
  app.get("/api/kaspa/test", (req, res) => {
    res.json({ 
  ...
  app.get("/api/kaspa/balance", async (req, res) => {
    try {
  ...
  app.post("/api/kaspa/fund-wallet", async (req, res) => {
    try {
      ...
      try {
        ...
        const txResult = await fundWallet(targetAddress, amount);
  ...
  app.get("/api/kaspa/generate-wallet", async (req, res) => {
    try {
  ...
  app.get("/api/database/health", authenticateToken, async (req, res) => {
    try {
  ...
  app.get("/api/backup/configuration", authenticateToken, async (req, res) => {
    try {
  ...
  app.put("/api/backup/configuration", authenticateToken, async (req, res) => {
    try {
  ...
  app.post("/api/backup/verify", authenticateToken, async (req, res) => {
    try {
  ...
  app.get("/api/data-retention/policies", authenticateToken, async (req, res) => {
    try {
  ...
  app.get("/api/data-retention/metrics", authenticateToken, async (req, res) => {
    try {
  ...
  app.put("/api/data-retention/policies/:policyId", authenticateToken, async (req, res) => {
    try {
  ...
  app.post("/api/data-retention/cleanup", authenticateToken, async (req, res) => {
    try {
  ...
  app.post("/api/data-retention/policies", authenticateToken, async (req, res) => {
    try {
  ...
  app.delete("/api/data-retention/policies/:policyId", authenticateToken, async (req, res) => {
    try {
      const { policyId } = req.params;
  ...
  app.post('/api/company/create-wallet', async (req, res) => {
    const { companyId } = req.body;
    if (!companyId) return res.status(400).json({ error: 'companyId required' });

    // Use the generateWallet function properly
    const wallet = await generateWallet();
    const mnemonic = wallet.privateKey; // If mnemonic is not available, use privateKey for demo
    const address = wallet.address;
    const privKey = wallet.privateKey;

    saveWallet(companyId, mnemonic, address, privKey);
    res.json({ mnemonic, address });
  });

  // Sign event endpoint
  app.post('/api/company/sign-event', (req, res) => {
    const { companyId, event } = req.body;
    if (!companyId || !event) return res.status(400).json({ error: 'companyId and event required' });

    let db: Record<string, any> = {};
    if (fs.existsSync(WALLET_DB)) db = JSON.parse(fs.readFileSync(WALLET_DB, 'utf8'));
    const wallet = (db as Record<string, any>)[companyId];
    if (!wallet) return res.status(404).json({ error: 'Wallet not found' });

    const privKey = decrypt(wallet.privKey);
    const eventData = JSON.stringify(event);
    const eventHash = require('crypto').createHash('sha256').update(eventData).digest();
    const { sign } = require('@noble/secp256k1');
    const signature = sign(eventHash, privKey);

    res.json({ signature: Buffer.from(signature).toString('hex'), address: wallet.address });
  });
