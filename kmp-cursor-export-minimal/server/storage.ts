...
export class DatabaseStorage implements IStorage {
  ...
  async getTodaysEvents(): Promise<Event[]> {
    ...
    return await db.select().from(events).where(eq(events.ts, todayTimestamp));
  }
  ...
  async getPolicyAudits(companyId?: string, limit: number = 50): Promise<PolicyAudit[]> {
    let query = db.select().from(policyAudits);
    
    if (companyId) {
      query = query.where(eq(policyAudits.companyId, companyId));
    }
  ...
  async getSystemAlerts(acknowledged?: boolean, limit: number = 50): Promise<SystemAlert[]> {
    ...
    if (acknowledged !== undefined) {
      query = query.where(eq(systemAlerts.acknowledged, acknowledged));
    }
  ...
  async getSystemAnalytics(startDate: Date, endDate: Date): Promise<any> {
    ...
    const [feeMetrics] = await db.select({
    ...
    return {
      ...
      totalFeesCollected: feeMetrics.totalFees || 0,
      averageFeePerEvent: feeMetrics.averageFee || 0,
      blockchainSuccessRate: successRate.successRate || 0,
      systemUptime: await this.getRealSystemUptime(),
      averageEventProcessingTime: await this.getRealEventProcessingTime(),
      systemThroughput: await this.getRealSystemThroughput(),
      monthlyRevenue: (feeMetrics.totalFees || 0) * 30 / 
  ...
  async getNotifications(userId?: number, companyId?: string, limit: number = 50): Promise<Notification[]> {
    ...
    // For company portal, prioritize companyId over userId
    if (companyId) {
      query = query.where(eq(notifications.companyId, companyId));
    } else if (userId) {
  ...
  async markAllNotificationsAsRead(userId?: number, companyId?: string): Promise<void> {
    let query = db.update(notifications).set({ read: true, updatedAt: new Date() });
  ...
  async getUnreadNotificationCount(userId?: number, companyId?: string): Promise<number> {
    let query = db.select({ count: sql<number>`COUNT(*)` }).from(notifications);
    
    // For company portal, prioritize companyId over userId
    if (companyId) {
      query = query.where(and(eq(notifications.companyId, companyId), eq(notifications.read, false)));
    } else if (userId) {
